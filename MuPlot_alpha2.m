...
% Last Modified by GUIDE v2.5 10-Aug-2015 11:42:52
...
handles.pathdir = pwd;
% Sets the Origin template path to this as default, you can edit this default
% setting here, or place files into this path with the GUI files
set(handles.originpathbox,'String',[ pwd '\Origin Templates']);
...
% Set current axes to handles.axes1, and then set the font size for labels
axes(handles.axes1);
set(gca,'fontsize',14);
% Clear the axes
cla;

%set the Origin template project path from the GUI text box 'handles.originpathbox'
%By default, this should be set to the rootdir
...
%     invoke(originObj,'Execute','doc -m;'); %Make origin session visible
    ...
    for a=1:i
        cd(handles.rootdir) %keep pwd as rootdir to prevent GUI error
        if i==1
            [material, solvent, conc, method, d, index, dlength, dwidth, T, Vd, Vg, I_D] = readSuperDuper([handles.PathName handles.cellname]);
            %         fileID = fopen(handles.cellname);
            current_file=handles.cellname;
        else
            [material, solvent, conc, method, d, index, dlength, dwidth, T, Vd, Vg, I_D] = readSuperDuper([handles.PathName handles.cellname{a}]);
        ...
        set(handles.diel_thck_box,'String',[num2str(d) ' m']);
        set(handles.material_box,'String',material);
        set(handles.solvent_box,'String',solvent);
        set(handles.conc_box,'String',[conc ' mM/L']);
        set(handles.method_box,'String',method);
        set(handles.temperature_box,'String',[num2str(T) ' K']);
        ...
        set(handles.yesgood,'Visible','On')
        set(handles.nogood,'Visible','On')
        set(handles.pickpts,'Visible','On')
        ...
        WSname = strcat('Mu',index,dlength,'L',num2str(a));
        ...
        %mobility, mu, Vt, subVt, and Ion/Ioff. 
        %======================================================================
        cla;
        hold on;
        ...
        text(xt,yt,textmu)
        % Resets the buttons for keeping data
        set(handles.yesgood,'Value',0)
        set(handles.nogood,'Value',0)
        set(handles.pickpts,'Value',0)
        set(handles.usabletext,'Visible','On')
        set(handles.yesgood,'Visible','On')
        set(handles.nogood,'Visible','On')
        set(handles.pickpts,'Visible','On')
        guidata(hObject, handles);
       ...
       uiwait(MuPlot_alpha2);
        
        %     if handles.keepques==2
        if get(handles.nogood,'Value') == 1
            %         close all
            continue
        elseif get(handles.pickpts,'Value') == 1
            %         figure,
            cla;
        ...
        %     keepques = menu('Is this usable data?','Yes','No','Pick Pts by Hand');
        %     ppos = get(MuPlot_alpha2,'Position');
...
% --- Executes on button press in yesgood.
function yesgood_Callback(hObject, eventdata, handles)
% hObject    handle to yesgood (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
uiresume(gcbf)
handles.keepques = 2;
...
% --- Executes on button press in nogood.
function nogood_Callback(hObject, eventdata, handles)
% hObject    handle to nogood (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
uiresume(gcbf)
handles.keepques = 2;
...
% Hint: get(hObject,'Value') returns toggle state of nogood


% --- Executes on button press in pickpts.
function pickpts_Callback(hObject, eventdata, handles)
% hObject    handle to pickpts (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
uiresume(gcbf)
handles.keepques = 2;
...
% Hint: get(hObject,'Value') returns toggle state of pickpts
